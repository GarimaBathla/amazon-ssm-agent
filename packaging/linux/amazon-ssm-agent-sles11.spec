...
%config(noreplace) /etc/init.d/amazon-ssm-agent

# GarimaBathla commenting below 2 lines as they are not needed for sles11
...
# GarimaBathla For SLES 11, use service to stop the ssm-agent.
...
if [ $1 -eq 0 ] ; then
    service stop amazon-ssm-agent
    chkconfig --set amazon-ssm-agent  off
    rm /etc/init.d/amazon-ssm-agent
#    /sbin/init --version &> stdout.txt
#    if [[ `cat stdout.txt` =~ upstart ]]; then
#        /sbin/stop amazon-ssm-agent
#        sleep 1
